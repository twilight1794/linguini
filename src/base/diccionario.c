...
int Dicc_Anadir(Dicc *d, char *l, void *v){
  if (d != NULL){
     ...
     */
    if (d->indices > (Dicc_Hash) (POT2(d->capacidad)/3*2)) op = Dicc_Extender(d);
...
int Dicc_Vaciar(Dicc *d){
  if (d != NULL){
    if (d->tamano){
      Dicc_Hash i;
      for (i=0; i<(Dicc_Hash) POT2(d->capacidad); i++){
